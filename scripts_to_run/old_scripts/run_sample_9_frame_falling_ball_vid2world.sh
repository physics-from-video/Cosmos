...
SEED_LIST=($(shuf --input-range=1-10000 --random-source=<(openssl enc -aes-256-ctr -pass pass:"$ACTUAL_SEED" -nosalt </dev/zero 2>/dev/null) -n 5)) # Generate 100 random seeds
...
for SEED in "${SEED_LIST[@]}"; do
    ...
    sbatch --job-name=enhanced_falling_ball_single_frame_vid2world_${SEED} sample_single_frame_vid2world.job \
        ...
        --batch_input_path cosmos1/models/diffusion/assets/v1p0/morpheus/multi-frame/enhanced_falling_ball_regular_wo_ps.jsonl \
